...
class BotRegistry {
  ...
  _buildLookups(bots) {
    ...
    for (const [agentId, bot] of Object.entries(bots)) {
      ...
      // 同时建立 owner openId 到 bot 的反向映射
      if (bot.owner?.openId) {
        this._byOpenId.set(bot.owner.openId, { ...bot, _isOwnerLookup: true });
      }
  ...
  // 获取所有人员信息（从 members 表）
  getMembers() {
    try {
      const data = JSON.parse(require('fs').readFileSync(
        require('path').join(__dirname, '..', 'data', 'wiki-bots.json'), 'utf8'
      ));
  ...
  findMemberByOpenId(openId) {
    ...
    return members.find(m => m.openId === openId) || null;
  }
